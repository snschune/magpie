...
MDRunBase::mapMDParticles()
{
                                    ...
                                    mesh_base.active_semilocal_elements_end()))
  {
    ...
    for (auto & p : indices_dist)
    {
      ...
      unsigned int particle_id = _md_particles.id[p.first];
      ...
      {
        // if this particle has not been assigned to an element yet, simply assign it
        // otherwise we need to override the old entry to avoid double counting
        if (_md_particles.elem_id[p.first] == libMesh::DofObject::invalid_unique_id)
        {
          if (_elem_particles.find(elem->unique_id()) != _elem_particles.end())
            _elem_particles[elem->unique_id()].push_back(particle_id);
          else
            _elem_particles[elem->unique_id()] = {particle_id};
        }
        else
        {
          // entry in _elem_particles should exist but guard w/ assert
          mooseAssert(_elem_particles.find(elem->unique_id()) != _elem_particles.end(),
                      "Entry in _elem_particles should exist.");

          for (auto & i : _elem_particles[elem->unique_id()])
            if (i == particle_id)
              i = particle_id;
        }

        // re-assigning the element id happens regardless
        _md_particles.elem_id[p.first] = elem->unique_id();
      }
    }
  }
}
